...
class Kiwoom:
    ...
    # OpenAPI+ 메서드
    #-------------------------------------------------------------------------------------------------------------------
    # 블로킹 효과: 로그인 요청하고, 로그인 결과 올 때까지 다른 것 실행하지 않고 멈춤
    def CommConnect(self, block=True): 
        ...
        :return: None
        """
        self.ocx.dynamicCall("CommConnect()") # CommConnect는 비동기 함수 => 서버 요청시 바로 return
        if block:
            while not self.connected:
                pythoncom.PumpWaitingMessages() # 블로킹 와중에도 메시지 큐는 순환이 되어야 함
