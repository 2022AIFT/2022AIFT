...
if __name__ == "__main__":
    ...
    df_kodex200 = pd.DataFrame(kodex200.min_data, columns=['date', 'open', 'high', 'low', 'close', 'volume', 'trade_volume'])
    sorted_kodex200 = df_kodex200.sort_values('date').reset_index(drop=True)
    sorted_kodex200.to_csv('collectKODEX200.csv')
    
    kodexInverse = minute_data()
    kodexInverse.rq_mindata("114800", 1, 1)
    df_kodexInverse = pd.DataFrame(kodexInverse.min_data, columns=['date', 'open', 'high', 'low', 'close', 'volume', 'trade_volume'])
    sorted_kodexInverse = df_kodexInverse.sort_values('date').reset_index(drop=True)
    sorted_kodexInverse.to_csv('collectKODEXInverse.csv')
